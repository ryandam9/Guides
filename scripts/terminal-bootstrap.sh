#!/usr/bin/env bash
# Terminal / HTML5 gateway bootstrap
# 8-color safe. No sudo required.
# Recommended usage: source ./terminal-bootstrap.sh
...
if [ -z "${BASH_VERSION:-}" ]; then
  echo "ERROR: terminal-bootstrap.sh must be run with Bash." >&2
  echo "Use one of these:" >&2
  echo "  bash ./scripts/terminal-bootstrap.sh --dry-run" >&2
  echo "  bash ./scripts/terminal-bootstrap.sh" >&2
  echo "For current-shell prompt/aliases, start Bash first, then source it:" >&2
  echo "  bash" >&2
  echo "  source ./scripts/terminal-bootstrap.sh" >&2
...
for arg in "$@"; do
  case "$arg" in
    ...
    --version|-V)
      echo "terminal-bootstrap v$TB_VERSION"
      restore_shell_opts
    ...
    --help|-h)
      cat <<'HELP'
Usage:
  source ./terminal-bootstrap.sh [--dry-run]
  bash   ./terminal-bootstrap.sh [--dry-run]
  bash   ./terminal-bootstrap.sh --uninstall [--dry-run]
  bash   ./terminal-bootstrap.sh --version

Recommended:
  source ./terminal-bootstrap.sh
...
Safety:
  ...
  - Existing .bashrc/.vimrc/.inputrc/.tmux.conf content is preserved.
...
Uninstall:
  --uninstall removes the managed blocks from .bashrc/.vimrc/.inputrc/
  .tmux.conf and deletes the generated theme/activation files.
...
# --- Hadoop ecosystem: Hive/Beeline, YARN, Oozie, HBase ----------------
# One-time per box (put above the managed block in .bashrc, or run ad hoc):
#   export BEELINE_URL='jdbc:hive2://host:10000/default;principal=hive/_HOST@REALM'
#   export OOZIE_URL='http://oozie-host:11000/oozie'
# Kerberos clusters need a ticket first: kinit user@REALM

__tb_need(){
  command -v "$1" >/dev/null 2>&1 && return 0
  echo "${2:-$1}: requires the '$1' CLI, which is not on this box" >&2
  return 127
}
__tb_confirm(){
  local reply
  printf '%s [y/N] ' "$1" >&2
  read -r reply
  case "$reply" in y|Y|yes|YES) return 0 ;; *) echo 'cancelled' >&2; return 1 ;; esac
}

# Beeline / Hive ---------------------------------------------------------
__tb_beeline(){
  __tb_need beeline || return
  [ -n "${BEELINE_URL:-}" ] || {
    echo "set BEELINE_URL first, e.g. export BEELINE_URL='jdbc:hive2://host:10000/default'" >&2
    return 2
  }
  beeline -u "$BEELINE_URL" --silent=true "$@"
}
bl(){ __tb_beeline "$@"; }                       # interactive beeline session
blq(){                                           # run one query, table output
  [ -n "$1" ] || { echo 'usage: blq "<sql>"' >&2; return 2; }
  __tb_beeline --outputformat=table -e "$1"
}
blf(){                                           # run a .sql file
  [ -f "$1" ] || { echo 'usage: blf <file.sql>' >&2; return 2; }
  __tb_beeline --outputformat=table -f "$1"
}
blcsv(){                                         # csv to stdout: blcsv "sql" > out.csv
  [ -n "$1" ] || { echo 'usage: blcsv "<sql>"  (redirect stdout to a file)' >&2; return 2; }
  __tb_beeline --outputformat=csv2 --showHeader=true -e "$1"
}
hive_dbs(){ blq "SHOW DATABASES"; }
hive_tables(){ blq "SHOW TABLES${1:+ IN $1}"; }
hive_desc(){ [ -n "$1" ] || { echo 'usage: hive_desc <db.table>' >&2; return 2; }; blq "DESCRIBE FORMATTED $1"; }
hive_ddl(){ [ -n "$1" ] || { echo 'usage: hive_ddl <db.table>' >&2; return 2; }; blq "SHOW CREATE TABLE $1"; }
hive_parts(){                                    # latest partitions (default 25)
  [ -n "$1" ] || { echo 'usage: hive_parts <db.table> [n]' >&2; return 2; }
  blq "SHOW PARTITIONS $1" | tail -n "${2:-25}"
}
hive_sample(){ [ -n "$1" ] || { echo 'usage: hive_sample <db.table> [n]' >&2; return 2; }; blq "SELECT * FROM $1 LIMIT ${2:-10}"; }
hive_count(){ [ -n "$1" ] || { echo 'usage: hive_count <db.table>' >&2; return 2; }; blq "SELECT COUNT(*) AS row_count FROM $1"; }
hive_loc(){                                      # HDFS location of a table
  [ -n "$1" ] || { echo 'usage: hive_loc <db.table>' >&2; return 2; }
  blcsv "DESCRIBE FORMATTED $1" | awk -F',' '$1 ~ /^"?Location:?"?$/ {gsub(/"/,"",$2); print $2; exit}'
}
hive_size(){                                     # on-disk size of a table
  [ -n "$1" ] || { echo 'usage: hive_size <db.table>' >&2; return 2; }
  __tb_need hdfs hive_size || return
  local loc
  loc="$(hive_loc "$1")" && [ -n "$loc" ] || { echo "hive_size: could not resolve location of $1" >&2; return 1; }
  hdfs dfs -du -s -h "$loc"
}

# YARN --------------------------------------------------------------------
yarn_running(){                                  # running apps, optional filter
  __tb_need yarn yarn_running || return
  if [ -n "${1:-}" ]; then
    yarn application -list 2>/dev/null | grep -i -- "$1"
  else
    yarn application -list 2>/dev/null
  fi
}
yarn_apps(){                                     # recent apps in ANY state
  __tb_need yarn yarn_apps || return
  yarn application -list -appStates ALL 2>/dev/null | head -n "${1:-30}"
}
yarn_status(){
  [ -n "$1" ] || { echo 'usage: yarn_status <application-id>' >&2; return 2; }
  __tb_need yarn yarn_status || return
  yarn application -status "$1" 2>/dev/null
}
yarn_why(){                                      # just the failure diagnostics
  [ -n "$1" ] || { echo 'usage: yarn_why <application-id>' >&2; return 2; }
  __tb_need yarn yarn_why || return
  yarn application -status "$1" 2>/dev/null | sed -n '/Diagnostics/,$p'
}
yarn_logs(){                                     # full logs to less, or grep them
  [ -n "$1" ] || { echo 'usage: yarn_logs <application-id> [grep-pattern]' >&2; return 2; }
  __tb_need yarn yarn_logs || return
  if [ -n "${2:-}" ]; then
    yarn logs -applicationId "$1" 2>/dev/null | grep -i --color=auto -- "$2"
  else
    yarn logs -applicationId "$1" 2>/dev/null | less
  fi
}
yarn_kill(){
  [ -n "$1" ] || { echo 'usage: yarn_kill <application-id>' >&2; return 2; }
  __tb_need yarn yarn_kill || return
  __tb_confirm "Kill YARN application $1?" && yarn application -kill "$1"
}
yarn_queues(){ __tb_need yarn yarn_queues || return; yarn queue -status "${1:-default}" 2>/dev/null; }

# Oozie -------------------------------------------------------------------
__tb_oozie(){
  __tb_need oozie || return
  [ -n "${OOZIE_URL:-}" ] || {
    echo "set OOZIE_URL first, e.g. export OOZIE_URL='http://oozie-host:11000/oozie'" >&2
    return 2
  }
  local sub="$1"; shift
  oozie "$sub" -oozie "$OOZIE_URL" "$@"
}
oz_jobs(){ __tb_oozie jobs -len "${1:-20}"; }                          # recent workflows
oz_coords(){ __tb_oozie jobs -jobtype coordinator -len "${1:-20}"; }   # coordinators
oz_running(){ __tb_oozie jobs -filter status=RUNNING -len 50; }
oz_failed(){ __tb_oozie jobs -filter 'status=FAILED;status=KILLED' -len "${1:-20}"; }
oz_info(){ [ -n "$1" ] || { echo 'usage: oz_info <job-id>' >&2; return 2; }; __tb_oozie job -info "$1"; }
oz_errors(){                                      # failed/killed actions of a job
  [ -n "$1" ] || { echo 'usage: oz_errors <job-id>' >&2; return 2; }
  __tb_oozie job -info "$1" | grep -E 'ERROR|FAILED|KILLED'
}
oz_log(){ [ -n "$1" ] || { echo 'usage: oz_log <job-id>' >&2; return 2; }; __tb_oozie job -log "$1" | less; }
oz_def(){ [ -n "$1" ] || { echo 'usage: oz_def <job-id>' >&2; return 2; }; __tb_oozie job -definition "$1"; }
oz_rerun(){                                       # rerun FAILED nodes only
  [ -n "$1" ] || { echo 'usage: oz_rerun <workflow-id>' >&2; return 2; }
  __tb_confirm "Rerun failed nodes of $1?" || return 1
  __tb_oozie job -rerun "$1" -Doozie.wf.rerun.failnodes=true
}
oz_kill(){
  [ -n "$1" ] || { echo 'usage: oz_kill <job-id>' >&2; return 2; }
  __tb_confirm "Kill Oozie job $1?" || return 1
  __tb_oozie job -kill "$1"
}

# HBase -------------------------------------------------------------------
hb(){                                             # run shell command(s) non-interactively
  __tb_need hbase hb || return
  [ -n "$1" ] || { echo "usage: hb \"<shell command>\"   e.g. hb \"scan 'ns:t', {LIMIT => 5}\"" >&2; return 2; }
  printf '%s\n' "$@" | hbase shell -n 2>/dev/null
}
hb_status(){ hb "status 'summary'"; }
hb_ns(){ hb "list_namespace"; }
hb_tables(){ hb "list${1:+ '$1'}"; }              # optional regex, e.g. hb_tables 'ns:.*'
hb_desc(){ [ -n "$1" ] || { echo 'usage: hb_desc <table>' >&2; return 2; }; hb "describe '$1'"; }
hb_scan(){ [ -n "$1" ] || { echo 'usage: hb_scan <table> [n]' >&2; return 2; }; hb "scan '$1', {LIMIT => ${2:-10}}"; }
hb_get(){
  [ -n "$1" ] && [ -n "$2" ] || { echo 'usage: hb_get <table> <rowkey>' >&2; return 2; }
  hb "get '$1', '$2'"
}
hb_count(){                                       # full scan — slow on big tables
  [ -n "$1" ] || { echo 'usage: hb_count <table> [progress-interval]' >&2; return 2; }
  echo "hb_count: full table scan — for big tables prefer: hbase org.apache.hadoop.hbase.mapreduce.RowCounter '$1'" >&2
  hb "count '$1', INTERVAL => ${2:-100000}"
}
...
write_vimrc() {
  local file="$TB_HOME/.vimrc" start='" >>> TERMINAL_BOOTSTRAP >>>' end='" <<< TERMINAL_BOOTSTRAP <<<'
  append_managed_block "$file" "$start" "$end" "vim settings" <<VIM
" >>> TERMINAL_BOOTSTRAP >>>
" terminal-bootstrap v$TB_VERSION
syntax on
set background=dark
...
write_tmux_conf() {
  local file="$TB_HOME/.tmux.conf" start='# >>> TERMINAL_BOOTSTRAP >>>' end='# <<< TERMINAL_BOOTSTRAP <<<'
  append_managed_block "$file" "$start" "$end" "8-color tmux settings" <<TMUX
# >>> TERMINAL_BOOTSTRAP >>>
# terminal-bootstrap v$TB_VERSION
...
uninstall() {
  ...
  local vim_start='" >>> TERMINAL_BOOTSTRAP >>>' vim_end='" <<< TERMINAL_BOOTSTRAP <<<'
  local f name start end
  ...
  for name in .bashrc .inputrc .tmux.conf .vimrc; do
    f="$TB_HOME/$name"
    [ -f "$f" ] || continue
    if [ "$name" = .vimrc ]; then start="$vim_start" end="$vim_end"; else start="$rc_start" end="$rc_end"; fi
    if ! validate_managed_markers "$f" "$start" "$end"; then
      err "refusing to touch $f: unbalanced or duplicate managed markers"
      continue
    fi
    if [ -z "$(extract_managed_block "$f" "$start" "$end")" ]; then
    ...
    if remove_managed_block "$f" "$start" "$end"; then
...
summary() {
  echo
  echo '============================================================'
  echo 'Terminal bootstrap summary'
  echo '============================================================'
  ...
  echo "Vim rc          : $TB_HOME/.vimrc"
  echo "Input rc        : $TB_HOME/.inputrc"
  echo "tmux conf       : $TB_HOME/.tmux.conf"
  ...
  echo 'Hadoop       : bl ; blq ; blcsv ; hive_* ; yarn_* ; oz_* ; hb_*   (see --help / docs)'
  echo '============================================================'
}
...
main() {
  ...
  for cmd in bash id getent tput awk sed hostname curl git vim tmux ps free ss ip find grep tail tar unzip column timeout xmllint beeline yarn oozie hbase hdfs python3 python; do cmd_check "$cmd"; done
